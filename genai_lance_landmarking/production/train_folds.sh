...
job() {
  ...
  resume=(); [[ -f $d/last.pt ]] && resume=(--resume)
  {
    ...
    OMP_NUM_THREADS=$T $PY train.py --angle "$view" --manifest manifest_v14.csv --folds folds_v14.json --fold "$k" \
      --epochs "$E" --batch-size 8 --loss dsnt --lr-decay cosine --cache-images --num-workers 0 \
      --out-dir "$d" "${resume[@]}" &&
    OMP_NUM_THREADS=$T $PY evaluate.py --angle "$view" --manifest manifest_v14.csv --folds folds_v14.json --fold "$k" \
      --checkpoint "$d/best.pt" --loss dsnt --out "$d/eval_test.json"
    echo "=== $view fold $k end $(date) status $?"
  } >> "$d/run.log" 2>&1
  tail -n 1 "$d/run.log"
}
export -f job; export MODEL PY OUT T E
